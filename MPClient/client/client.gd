...
func _client_close_request(code, reason):
	chat_utils._log(_log_dest, "Close code: %d, reason: %s" % [code, reason])

func _peer_connected(id):
	chat_utils._log(_log_dest, "%s: Client just connected" % id)
...
func _client_connected(protocol):
	chat_utils._log(_log_dest, "Client just connected with protocol: %s" % protocol)
	_client.get_peer(1).set_write_mode(_write_mode)

func _client_disconnected(clean=true):
	chat_utils._log(_log_dest, "Client just disconnected. Was clean: %s" % clean)

func _client_received(p_id = 1):
	if _use_multiplayer:
		var peer_id = _client.get_packet_peer()
		var packet = _client.get_packet()
		chat_utils._log(_log_dest, "MPAPI: From %s Data: %s" % [str(peer_id), chat_utils.decode_data(packet, false)])
	else:
		var packet = _client.get_peer(1).get_packet()
		var is_string = _client.get_peer(1).was_string_packet()
		chat_utils._log(_log_dest, "Received data. BINARY: %s: %s" % [not is_string, chat_utils.decode_data(packet, is_string)])
...
func send_data(data, dest):
	...
	if _use_multiplayer:
		...
		_client.put_packet(chat_utils.encode_data(data, _write_mode))
	else:
		_client.get_peer(1).put_packet(chat_utils.encode_data(data, _write_mode))
