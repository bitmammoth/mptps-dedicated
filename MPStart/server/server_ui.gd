...
func _ready():
	...
	if _server.listen(port, supported_protocols, use_multiplayer) == OK:
		chat_utils._log(_log_dest, "Listing on port %s" % port)
		if not use_multiplayer:
			chat_utils._log(_log_dest, "Supported protocols: %s" % supported_protocols)
	else:
		chat_utils._log(_log_dest, "Error listening on port %s" % port)
...
func _on_Send_pressed():
	...
	chat_utils._log(_log_dest, "Sending data %s to %s" % [_line_edit.text, dest])
