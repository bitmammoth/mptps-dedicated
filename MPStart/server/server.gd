...
func _client_close_request(id, code, reason):
	...
	chat_utils._log(_log_dest, "Client %s close code: %d, reason: %s" % [id, code, reason])
...
func _client_connected(id, protocol):
	...
	chat_utils._log(_log_dest, "%s: Client connected with protocol %s" % [id, protocol])

func _client_disconnected(id, clean = true):
	chat_utils._log(_log_dest, "Client %s disconnected. Was clean: %s" % [id, clean])
	if _clients.has(id):
		_clients.erase(id)
...
func _client_receive(id):
	if _use_multiplayer:
		...
		chat_utils._log(_log_dest, "MPAPI: From %s data: %s" % [peer_id, chat_utils.decode_data(packet, false)])
	else:
		var packet = _server.get_peer(id).get_packet()
		var is_string = _server.get_peer(id).was_string_packet()
		chat_utils._log(_log_dest, "Data from %s BINARY: %s: %s" % [id, not is_string, chat_utils.decode_data(packet, is_string)])

func send_data(data, dest):
	if _use_multiplayer:
		_server.set_target_peer(dest)
		_server.put_packet(chat_utils.encode_data(data, _write_mode))
	else:
		for id in _clients:
			_server.get_peer(id).put_packet(chat_utils.encode_data(data, _write_mode))
